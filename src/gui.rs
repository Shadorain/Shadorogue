...
use super::camera;
use specs::prelude::*;
use super::{Pools, HungerState, gamelog::GameLog, Map, Name, Position, InBackpack,
    State, Viewshed, RunState, Equipped, HungerClock, Attribute, Attributes,
    rex_assets::RexAssets, Hidden, Consumable, Item, Vendor, VendorMode, MagicItem,
    MagicItemClass
};
...
pub fn draw_attribute (name: &str, attribute: &Attribute, y:i32, ctx: &mut Rltk) {
    let black = RGB::named(rltk::BLACK);
    let attr_gray: RGB = RGB::from_hex("#CCCCCC").expect("Oops");
    ctx.print_color(50, y, attr_gray, black, name);
    let color: RGB = if attribute.modifiers < 0 { RGB::from_f32(1.0, 0.0, 0.0) }
...
pub fn draw_ui (ecs: &World, ctx: &mut Rltk) {
    ...
    /* UI boxes */
    draw_hollow_box(ctx, 0, 0, 79, 59,  box_gray, black); /* Main Box */
    ...
    ctx.draw_bar_horizontal(64, 1, 14, player_pools.hit_points.current, player_pools.hit_points.max, RGB::named(rltk::RED), black);
    ctx.draw_bar_horizontal(64, 2, 14, player_pools.mana.current, player_pools.mana.max, RGB::named(rltk::BLUE), black);

    /* Level */
    format!("Level: {}", player_pools.level);
    ctx.print_color(50, 3, white, black, &xp);
    let xp_level_start = (player_pools.level-1) * 1000;
    ctx.draw_bar_horizontal(64, 3, 14, player_pools.xp - xp_level_start, 1000, RGB::named(rltk::GOLD), black);
    ...
    /* Initiative + Weight */
    ctx.print_color(50, 9, white, black, &format!("{:.0} lbs ({} lbs max)",
    ...
    for (ent, equipped_by, item_name) in (&entities, &equipped, &name).join() {
        if equipped_by.owner == *player_entity {
            ctx.print_color(50, y, get_item_color(ecs, ent), black, &item_name.name);
            y += 1;
        }
    };
    ...
    let mut index = 1;
    for (ent, carried_by, _consumable, item_name) in (&entities, &backpack, &consumables, &name).join() {
        if carried_by.owner == *player_entity && index < 10 {
            ctx.print_color(50, y, yellow, black, &format!("↑{}", index));
            ctx.print_color(53, y, get_item_color(ecs, ent), black, &item_name.name);
            y += 1;
            index += 1;
        }
    };
...
fn draw_tooltips (ecs: &World, ctx : &mut Rltk) {
    ...
    for (ent, name, pos, _hidden) in (&entities, &names, &positions, !&hidden).join() {
        if pos.x == mouse_map_pos.0 && pos.y == mouse_map_pos.1 {
            let mut tip = Tooltip::new();
            tip.add(name.name.to_string());
            /* Comment on attributes */
            let attr = attributes.get(ent);
...
pub fn show_inventory (gs : &mut State, ctx : &mut Rltk) -> (ItemMenuResult, Option<Entity>) {
    ...
    let mut j = 0;
    for (ent,_pack,name) in (&entities, &backpack, &names).join().filter(|item| item.1.owner == *player_entity) {
        ctx.set(17, y, RGB::named(rltk::WHITE), RGB::named(rltk::BLACK), rltk::to_cp437('('));
        ctx.set(18, y, RGB::named(rltk::YELLOW), RGB::named(rltk::BLACK), 97+j as rltk::FontCharType);
        ctx.set(19, y, RGB::named(rltk::WHITE), RGB::named(rltk::BLACK), rltk::to_cp437(')'));

        ctx.print_color(21, y, get_item_color(&gs.ecs, ent), RGB::from_f32(0.0, 0.0, 0.0), &name.name.to_string());
        equipable.push(ent);
        y += 1;
        j += 1;
    };
...
pub fn remove_equipment_menu (gs : &mut State, ctx : &mut Rltk) -> (ItemMenuResult, Option<Entity>) {
    ...
    let mut y = (25-(count/2)) as i32;
    ctx.draw_box(15, y-2, 50, (count+3) as i32, RGB::named(rltk::WHITE), RGB::named(rltk::BLACK));
    ctx.print_color(18, y-2, RGB::named(rltk::YELLOW), RGB::named(rltk::BLACK), "Remove which piece of Equipment?");
    ctx.print_color(18, y+count as i32+1, RGB::named(rltk::YELLOW), RGB::named(rltk::BLACK), "ESCAPE to Cancel");
        ...
        match ctx.key {
            ...
            Some(key) => {
                match key {
                    ...
                    VirtualKeyCode::Return | VirtualKeyCode::L => return MainMenuResult::Selected { selected: selection },
...
pub fn show_cheat_menu (_gs: &mut State, ctx: &mut Rltk) -> CheatMenuResult {
    ...
    ctx.set(18, y, yellow, black, rltk::to_cp437('T'));
    ctx.set(19, y, white, black, rltk::to_cp437(')'));
    ctx.print(21, y, "Teleport to exit");

    y += 1;
    ctx.set(17, y, white, black, rltk::to_cp437('('));
    ctx.set(18, y, yellow, black, rltk::to_cp437('H'));
    ctx.set(19, y, white, black, rltk::to_cp437(')'));
    ctx.print(21, y, "Heal all wounds");

    y += 1;
    ctx.set(17, y, white, black, rltk::to_cp437('('));
    ctx.set(18, y, yellow, black, rltk::to_cp437('R'));
    ctx.set(19, y, white, black, rltk::to_cp437(')'));
    ctx.print(21, y, "Reveal the map");

    y += 1;
    ctx.set(17, y, white, black, rltk::to_cp437('('));
    ctx.set(18, y, yellow, black, rltk::to_cp437('G'));
    ctx.set(19, y, white, black, rltk::to_cp437(')'));
...
pub fn game_over (ctx: &mut Rltk) -> GameOverResult {
    ...
    ctx.print_color_centered(18, RGB::named(rltk::WHITE), RGB::named(rltk::BLACK), "That day isnt here yet! but coming...");
...
fn vendor_sell_menu (gs: &mut State, ctx: &mut Rltk, _vendor: Entity, _mode: VendorMode) -> (VendorResult, Option<Entity>, Option<String>, Option<f32>) {
    ...
    let mut j = 0;
    for (ent, _pack, name, item) in (&entities, &backpack, &names, &items).join().filter(|item| item.1.owner == *player_entity ) {
        ctx.set(17, y, RGB::named(rltk::WHITE), RGB::named(rltk::BLACK), rltk::to_cp437('('));
        ctx.set(18, y, RGB::named(rltk::YELLOW), RGB::named(rltk::BLACK), 97+j as rltk::FontCharType);
        ctx.set(19, y, RGB::named(rltk::WHITE), RGB::named(rltk::BLACK), rltk::to_cp437(')'));
