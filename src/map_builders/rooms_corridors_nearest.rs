        ...
        for (i, room) in rooms.iter().enumerate() {
            ...
            for (j, other_room) in rooms.iter().enumerate() {
                if i != j && !connected.contains(&j) {
                    ...
                    let dist = rltk::DistanceAlg::Pythagoras.distance2d(room_center_pt, other_center_pt);
                    room_distance.push((j, dist));
                }
            };
            ...
            if !room_distance.is_empty() {
                ...
                let corridor = draw_corridor(&mut build_data.map, room_center.0,
                    room_center.1, dest_center.0, dest_center.1);
                connected.insert(i);
