...
use super::{InitialMapBuilder, BuilderMap, BuilderChain, Position, TileType};
...
pub fn town_builder (new_depth:i32, _rng: &mut rltk::RandomNumberGenerator, width:i32, height:i32) -> BuilderChain {
    let mut chain = BuilderChain::new(new_depth, width, height, "The Town of Shadoville");
...
impl TownBuilder {
    ...
    pub fn water_and_piers (&mut self, rng: &mut rltk::RandomNumberGenerator, build_data : &mut BuilderMap) {
    ...
    fn buildings (&mut self, rng: &mut rltk::RandomNumberGenerator, build_data: &mut BuilderMap,
    ...
    fn add_doors (&mut self, rng: &mut rltk::RandomNumberGenerator, build_data: &mut BuilderMap,
        buildings: &mut Vec<(i32, i32, i32, i32)>, wall_gap_y:i32) -> Vec<usize>
    {
    ...
    fn add_paths (&mut self, build_data: &mut BuilderMap, doors: &[usize]) {
        ...
        for door_idx in doors.iter() {
            ...
            let door_pt = rltk::Point::new(*door_idx as i32 % build_data.map.width as i32,
                *door_idx as i32 / build_data.map.width as i32);
            for r in roads.iter() {
                nearest_roads.push((*r, rltk::DistanceAlg::PythagorasSquared.distance2d(door_pt,
            ...
            let dest = nearest_roads[0].0;
            let path = rltk::a_star_search(*door_idx, dest, &mut build_data.map);
            if path.success {
        ...
        rng: &mut rltk::RandomNumberGenerator)
    {
        ...
        let mut to_place: Vec<&str> = vec!["Barkeep", "Shady Salesman", "Patron", "Patron", "Table", "Chair", "Table", "Chair"];
    ...
    fn build_temple (&mut self, building: &(i32, i32, i32, i32), build_data: &mut BuilderMap,
        rng: &mut rltk::RandomNumberGenerator)
    {
        let mut to_place: Vec<&str> = vec!["Priest", "Parishioner", "Chair", "Chair", "Candle", "Candle"];
    ...
    fn build_abandoned_house (&mut self, building: &(i32, i32, i32, i32), build_data: &mut BuilderMap,
        rng: &mut rltk::RandomNumberGenerator)
    {
            ...
            for x in building.0 .. building.0 + building.2 {
                ...
                if build_data.map.tiles[idx] == TileType::WoodFloor && idx != 0 {
                    ...
                    if rng.roll_dice(1, 6)==1 {
                        build_data.spawn_list.push((idx, "Dead Thing".to_string()));
                    }
                }
            };
        };
    ...
    fn spawn_townsfolk (&mut self, build_data: &mut BuilderMap, rng: &mut rltk::RandomNumberGenerator,
    ...
    fn random_building_spawn (&mut self, building: &(i32, i32, i32, i32), build_data: &mut BuilderMap,
        rng: &mut rltk::RandomNumberGenerator, to_place: &mut Vec<&str>, player_idx: usize)
    {
        for y in building.1 .. building.1 + building.3 {
            for x in building.0 .. building.0 + building.2 {
                let idx = build_data.map.xy_idx(x, y);
                if build_data.map.tiles[idx] == TileType::WoodFloor && idx != player_idx &&
                    rng.roll_dice(1, 3)==1 && !to_place.is_empty() {
    ...
    fn building_factory (&mut self, rng: &mut rltk::RandomNumberGenerator, build_data: &mut BuilderMap,
        buildings: &[(i32, i32, i32, i32)], building_index: &[(usize, i32, BuildingTag)])
    {
        for (i, building) in buildings.iter().enumerate() {
